...
function sha256Fallback(bufferLike) {
  ...
  function processBlock(view, offset) {
  ...
  for (let offset = 0; offset < fullBlocks * 64; offset += 64) {
    processBlock(view, offset);
  }

  const remLength = bytes.length - fullBlocks * 64;
  const remBlock = new Uint8Array(128);
  for (let i = 0; i < remLength; i++) remBlock[i] = bytes[fullBlocks * 64 + i];
  remBlock[remLength] = 0x80;

  const paddedRemLength = Math.ceil((remLength + 9) / 64) * 64;
  const remView = new DataView(remBlock.buffer);
  remView.setUint32(paddedRemLength - 8, Math.floor(bitLength / 0x100000000));
  remView.setUint32(paddedRemLength - 4, bitLength >>> 0);

  for (let offset = 0; offset < paddedRemLength; offset += 64) {
    processBlock(remView, offset);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchAdminJson(url, options = {}, promptMessage = '') {
  ...
  async function sendChunk(file, fileId, uploadConfig, chunkIndex, abortSignal, onRetry, onProgress) {
    const start = chunkIndex * CHUNK_SIZE;
    ...
    while (!abortSignal.aborted) {
      ...
      try {
        ...
        const response = await new Promise((resolve, reject) => {
          ...
          if (timeout.signal.aborted) return onAbort();
          ...
          xhr.open('POST', `/api/upload/chunk?${query.toString()}`);
          xhr.send(formData);
        });
        timeout.cleanup();
    ...
    try {
      ...
      await runPool(missingChunks, FILE_CHUNK_CONCURRENCY, async chunkIndex => {
        const bytes = await sendChunk(file, fileId, uploadConfig, chunkIndex, abortController.signal, null, (loadedBytes) => {
          ...
          updateProgressUi(card.progressBar, card.timeEl, Math.min(uploadedBytes + activeSum, file.size), file.size);
        });
      ...
      await runPool(chunkQueue, FOLDER_CHUNK_CONCURRENCY, async item => {
        const bytes = await sendChunk(
          ...
          (loadedBytes) => {
            ...
            let activeSum = 0;
            activeChunkProgress.forEach((val) => { activeSum += val; });
            updateProgressUi(card.progressBar, card.timeEl, Math.min(uploadedBytes + activeSum, totalSize), totalSize);
          }
        );
        activeChunkProgress.delete(item.task.fileId + '_' + item.chunkIndex);
        uploadedBytes += bytes;
        updateProgressUi(card.progressBar, card.timeEl, uploadedBytes, totalSize);
      });
